...
// Attach to a WPA2 network named SCU_IOT_COURSE (this is set in the wifi_config_dct.h file).
//
...
void application_start( )
{
    ...
    /* Cypress.com Address */
    wiced_hostname_lookup("www.scu.edu", &ipAddress, WICED_NEVER_TIMEOUT, WICED_STA_INTERFACE);
    WPRINT_APP_INFO(("SCU: "));
    printIp(ipAddress);
    ...
    {
        ledToBlink = WICED_SH_LED1; /* Blink LED1 if connection was successful */
    }
    else
    {
        ledToBlink = WICED_SH_LED0; /* Blink LED0 if connection was unsuccessful */
    }
